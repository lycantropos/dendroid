...
def test_properties(
    map_with_key: tuple[Map[KeyT, ValueT | None], KeyT],
) -> None:
...
@given(strategies.non_empty_maps_with_their_keys)
def test_step(map_with_key: tuple[Map[KeyT, ValueT | None], KeyT]) -> None:
    map_, key = map_with_key
    original = copy(map_)
...
def test_step_with_default(
    ...
    assert key in map_
    assert result in map_.values()
    assert result is original[key] if key in original else result is default
    assert len(map_) == len(original) + (key not in original)
