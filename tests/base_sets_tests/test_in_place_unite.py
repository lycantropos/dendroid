...
def test_connection_with_union(sets_pair: BaseSetsPair[ValueT]) -> None:
...
def test_properties(sets_pair: BaseSetsPair[ValueT]) -> None:
    ...
    assert all(value in left_set for value in right_set)
    assert not right_set or not left_set.isdisjoint(right_set)
