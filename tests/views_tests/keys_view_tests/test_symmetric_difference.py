...
def test_properties(keys_views_pair: KeysViewsPair[KeyT]) -> None:
    ...
    assert (
        ...
        <= min(
            to_height(left_keys_view._tree)
            + to_height(right_keys_view._tree)
            + 1,
            to_max_binary_tree_height(result_tree),
        )
    )
    assert all(
        (key in left_keys_view) is not (key in right_keys_view)
        for key in result
    )
    assert left_keys_view <= right_keys_view or not result.isdisjoint(
        left_keys_view
    )
    assert right_keys_view <= left_keys_view or not result.isdisjoint(
        right_keys_view
    )
...
@given(strategies.keys_views_triplets)
def test_repeated(keys_views_triplet: KeysViewsTriplet[KeyT]) -> None:
    left_keys_view, mid_tree, right_keys_view = keys_views_triplet
