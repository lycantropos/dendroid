...
def test_base_case(empty_keys_view_with_keys_view: KeysViewsPair) -> None:
...
def test_step(two_keys_views_with_value: Tuple[KeysView, KeysView, Key]
              ) -> None:
    ...
    assert implication(not left_keys_view.isdisjoint(right_keys_view),
                       not next_left_keys_view.isdisjoint(right_keys_view))
    assert implication(left_keys_view.isdisjoint(right_keys_view),
                       equivalence(next_left_keys_view
                                   .isdisjoint(right_keys_view),
...
def test_symmetry(keys_views_pair: KeysViewsPair) -> None:
    ...
    assert equivalence(first_keys_view.isdisjoint(second_keys_view),
