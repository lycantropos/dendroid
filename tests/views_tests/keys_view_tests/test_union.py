...
def test_properties(keys_views_pair: KeysViewsPair) -> None:
    ...
    assert (to_min_binary_tree_height(result_tree)
            ...
            <= min(to_height(left_keys_view.tree)
                   + to_height(right_keys_view.tree) + 1,
                   to_max_binary_tree_height(result_tree)))
    assert all(key in left_keys_view or key in right_keys_view
               for key in result)
    assert ((not left_keys_view or not result.isdisjoint(left_keys_view))
            and (not right_keys_view
...
def test_difference_operand(keys_views_triplet: KeysViewsTriplet) -> None:
    ...
    assert result == ((left_keys_view | right_keys_view)
                      - (mid_tree - right_keys_view))


@given(strategies.keys_views_triplets)
def test_distribution_over_intersection(keys_views_triplet: KeysViewsTriplet
                                        ) -> None:
    left_keys_view, mid_tree, right_keys_view = keys_views_triplet
...
def test_connection_with_subkeys_view_relation(keys_views_pair: KeysViewsPair
    ...
    assert left_keys_view <= result and right_keys_view <= result
...
def test_connection_with_disjoint(keys_views_pair: KeysViewsPair) -> None:
    ...
    assert equivalence(left_keys_view.isdisjoint(right_keys_view),
                       len(result)
                       == len(left_keys_view) + len(right_keys_view))
