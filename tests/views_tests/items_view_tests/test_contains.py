...
def test_base_case(
    items_view_with_item: tuple[ItemsView[KeyT, ValueT], Item[KeyT, ValueT]],
) -> None:
...
def test_step(
    items_view_with_items_pair: tuple[
    ...
    assert equivalence(
        item in next_items_view,
        (item in items_view and not are_items_keys_equal(item, extra_item))
