...
def test_properties(items_views_pair: ItemsViewsPair) -> None:
    ...
    assert (to_min_binary_tree_height(result_tree)
            ...
            <= min(to_height(left_items_view.tree) + to_height(
                    right_items_view.tree) + 1,
                   to_max_binary_tree_height(result_tree)))
    assert all((value in left_items_view) is not (value in right_items_view)
               for value in result)
...
@given(strategies.items_views_pairs)
def test_equivalent_using_union_of_differences(items_views_pair: ItemsViewsPair
                                               ) -> None:
    left_items_view, right_items_view = items_views_pair
