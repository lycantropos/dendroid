...
def test_base_case(
    empty_items_view_with_items_view: ItemsViewsPair[KeyT, ValueT],
) -> None:
...
def test_step(
    two_items_views_with_value: tuple[
    ...
    assert implication(
        not left_items_view.isdisjoint(right_items_view),
        not next_left_items_view.isdisjoint(right_items_view)
        or (len(left_items_view) == 1 and item not in left_items_view),
    )
    assert implication(
        left_items_view.isdisjoint(right_items_view),
        equivalence(
            next_left_items_view.isdisjoint(right_items_view),
...
@given(strategies.items_views_pairs)
def test_symmetry(items_views_pair: ItemsViewsPair[KeyT, ValueT]) -> None:
    first_items_view, second_items_view = items_views_pair

    assert equivalence(
        first_items_view.isdisjoint(second_items_view),
