...
def test_properties(items_views_pair: ItemsViewsPair[KeyT, ValueT]) -> None:
    ...
    assert (
        ...
        <= min(
            to_height(left_items_view.tree),
            to_height(right_items_view.tree),
            to_max_binary_tree_height(result_tree),
        )
    )
    assert all(
        value in left_items_view and value in right_items_view
        for value in result
    )
    assert not result or (
        not result.isdisjoint(left_items_view)
...
def test_idempotence(items_view: ItemsView[KeyT, ValueT]) -> None:
...
@given(strategies.items_views_pairs)
def test_commutativity(items_views_pair: ItemsViewsPair[KeyT, ValueT]) -> None:
    left_items_view, right_items_view = items_views_pair
...
def test_connection_with_subset_relation(
    ...
    assert result <= right_items_view
